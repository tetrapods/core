...
function Tetrapod() {
	...
	function register(type, contractName, structName, contractId, structId) {
		...
		protocol.reverseMap["" + contractId + "." + structId] = contractName
				+ "." + structName;
	}
	...
	function logResponse(result) {
		...
		console.log("[%d] <- %s%s", result._requestId, nameOf(result), JSON
				.stringify(result, dropUnderscored));
	}
	...
	function logMessage(result) {
		console.log("[M:%d] <- %s%s", result._topicId, nameOf(result), JSON
				.stringify(result, dropUnderscored));
	}
	...
	function onSocketMessage(event) {
		...
		if (result._requestId != null) {
			...
			if (self.commsLog)
				logMessage(result);
			var func = messageHandlers[result._contractId + "."
					+ result._structId];
			if (func) {
				func(result);
			}
